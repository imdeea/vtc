...
class bp_course_filters{
    ...
    function certificate_url($url,$course_id,$user_id){
    	...
    	if(!empty($url) || empty($course_id) || empty($user_id)){
    		return $url;
    	}
    	global $wpdb;
    	$url = $wpdb->get_var($wpdb->prepare("SELECT guid FROM {$wpdb->posts} WHERE post_type = 'attachment' AND post_author = %d AND post_parent = %d AND post_name = %s",$user_id,$course_id,'certificate_'.$course_id.'_'.$user_id.''));
